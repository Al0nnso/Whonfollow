        ...
        request.post({
                    ...
                    if(getuser_id!=null){
                        ...
                        if(_USER.token!=undefined){
                            ...
                            if(last_update>thisdate){
                                ...
                                return res.redirect(process.env.APP_URL+"/auth/?userid="+objbody.user_id+"&token="+_USER.token)
                            }else{
                ...
                async function get_longtoken(){
                    ...
                    request(longtoken_url, async function (error, response, body) {
                        ...
                        const redirect_url=process.env.APP_URL+"/auth/?userid="+objbody.user_id+"&token="+longtoken.access_token
                        if(getuser_id==null){
                            ...
                            request(getusername_url, async function (error, response, body) {
                                ...
                                request(getshortid_url, async function (error, response, body) {
                                    ...
                                    const user = await User.create({userid:objbody.user_id,shortid:userobj2.graphql.user.id,token:longtoken.access_token,expire_token: new Date(newDate)});
                                    console.log(user)
